...
namespace ConnectivitySample
{
    public partial class App : Application
    {
        public App()
        {
            ...
            MainPage = new ConnectivitySample.MainPage();
            var page = MainPage;
            ...
            connectivityButton.Clicked += async (sender, args) =>
            {
                ...
                try
                {
                    canReach1.Text = await CrossConnectivity.Current.IsReachable(host.Text)
                        ? "Reachable"
                        : "Not reachable";
                }
                catch (Exception ex)
                ...
                try
                {
                    canReach2.Text = await CrossConnectivity.Current.IsRemoteReachable(host2.Text, int.Parse(port.Text))
                        ? "Reachable"
                        : "Not reachable";
                }
                catch (Exception ex)
            ...
            {
                page.DisplayAlert("Connectivity Changed",
                    "IsConnected: " + CrossConnectivity.Current.IsConnected + "  Args:" +
                    args.IsConnected.ToString(), "OK");
            };

            CrossConnectivity.Current.ConnectivityTypeChanged += (sender, args) =>
            {
                page.DisplayAlert("Connectivity  Type Changed", "Types: " + args.ConnectionTypes.FirstOrDefault(),
                    "OK");
            };
